...
function findElementGreaterThan(arr, num) {
  if (Array.isArray(arr)) {
    const foundElement = arr.find((elem) => elem > num);
...
function findIndexByProperty(arr, name) {
  if (Array.isArray(arr)) {
    const foundIndex = (elem) => elem.name === name;
...
function findLastEvenNumber(arr) {
  if (Array.isArray(arr)) {
    const lastEvenNumber = arr.findLast((elem) => elem % 2 == 0);
    if (lastEvenNumber === undefined) {
...
function getSubArrayAndConvertToString(arr, startIdx) {
  if (Array.isArray(arr)) {
    const subArrayString = arr.slice(startIdx);
...
function getPositiveArrayValues(arr) {
  if (Array.isArray(arr)) {
    ...
    let positiveArrayValues = arr1.filter((elem) => elem > 0);
...
function removeAndDouble(arr, startIdx) {
  if (Array.isArray(arr)) {
    arr.splice(startIdx);
    let doubledArray = arr.map((elem) => elem + elem);
    return doubledArray;
  } else return "Вхідний параметр має бути масивом";
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
...
function flattenSquareAndFilter(arr, threshold) {
  if (Array.isArray(arr)) {
    let filteredSquaredArray = arr.flat();
    filteredSquaredArray = filteredSquaredArray.map((elem) => elem ** 2);
    filteredSquaredArray = filteredSquaredArray.filter(
      (elem) => elem > threshold
    );
...
function expandArrayByN(arr, n) {
  if (Array.isArray(arr) && typeof n == "number") {
    let expandedArray = arr.flatMap((item) => Array(n).fill(item));
    return expandedArray;
  } else return "Перший вхідний параметр має бути масивом, другий - числом";
}
...
function findLongestWord(arr) {
  if (Array.isArray(arr)) {
    let longestWord = arr.reduce((currentLongest, currentWord) => {
...
function findLastNegativeNumberIndex(arr) {
  ...
  const lastIndex = arr.findLastIndex((number) => number < 0);
  return lastIndex;
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні, повертаємо -1
